...
void reply_get (accepted_socket& client_sock, cxi_header& header) {
   ...
   if (status == -1) {
      outlog << "File does not exist on server" << endl;
      header.command = cxi_command::NAK;
      header.nbytes = htonl (errno);
      send_packet (client_sock, &header, sizeof header);
   }else {
      ...
      DEBUGF ('h', "sending header " << header);
      send_packet (client_sock, &header, sizeof header);
      send_packet (client_sock, buffer, nbytes);
      DEBUGF ('h', "sent " << nbytes << " bytes");

      delete(buffer);

      outlog << "File was sent succesfully" << endl;
   }
}

void reply_put (accepted_socket& client_sock, cxi_header& header) {
   size_t nbytes = htonl (header.nbytes);
   ...
   if (file.good()) {
      ...
      outlog << "Error in writing to the file" << endl;
      header.command = cxi_command::NAK;
      header.nbytes = htonl (errno);
   }
...
void reply_rm (accepted_socket& client_sock, cxi_header& header) {
   ...
   if (rc == 0) {
      outlog << "File was succesfully deleted" << endl;
      header.command = cxi_command::ACK;
   }else {
      outlog << "Error in deleting file" << endl;
      header.command = cxi_command::NAK;
      header.nbytes = htonl (errno);
   }
   send_packet (client_sock, &header, sizeof header);
