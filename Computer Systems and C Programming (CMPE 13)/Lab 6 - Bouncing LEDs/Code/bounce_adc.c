...
int main(void)
{
    ...
    OledInit(); // Initializes OLED
        
    while (1) {      
        int percentage = (int)(((float) data.value / 1023) * 100); // Change to float for decimal calculation, then back to int
        
        if (data.event == TRUE) {    
            sprintf(message, "Potentiometer Value: \n \t %d \n \t %.0d%%", data.value, percentage); // Stores string in message
            
            OledDrawString(message); // Draws string
            OledUpdate(); // Updates to OLED
        }
        data.event = 0;
    }
...
void __ISR(_ADC_VECTOR, IPL2AUTO) AdcHandler(void)
{
    ...
    // Average of the registers
    int average = (ADC1BUF0 + ADC1BUF2 + ADC1BUF3 + ADC1BUF4 + ADC1BUF5 + ADC1BUF6 + ADC1BUF7)/7;
    
    // Checks if averaged value has changed from last run
