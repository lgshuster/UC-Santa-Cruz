...
int main(void)
{
    ...
    for(;;)
    {
        ...
        if (ADC_output < 0) { // Check to make sure ADC_output doesn't overflow
            ADC_output = 0;
        }
        ...
        ADC_output/=182; // Degrees shifted
        shift = (ADC_output * 128) / 360; // Scale for shifting
        
        
        CyDmaTdSetAddress(DMA_2_TD[0], LO16((uint32)&sineTable[shift]), LO16((uint32)VDAC8_2_Data_PTR)); // Update sine wave from table
