...
class ForwardingUnit extends Module {
  ...
  when (io.rs1 === io.exmemrd && io.exmemrd =/= 0.U && io.exmemrw) { 
    io.forwardA := 1.U // Forward from mem stage
  } .elsewhen (io.rs1 === io.memwbrd && io.memwbrd =/= 0.U && io.memwbrw) {
    io.forwardA := 2.U // Forward from wb stage
  } .otherwise {
    io.forwardA := 0.U // Don't forward
  }

  // set forward B
  when (io.rs2 === io.exmemrd && io.exmemrd =/= 0.U && io.exmemrw) {
    io.forwardB := 1.U // Forward from mem stage
  } .elsewhen (io.rs2 === io.memwbrd && io.memwbrd =/= 0.U && io.memwbrw) {
    io.forwardB := 2.U // Forward from wb stage
  } .otherwise {
    io.forwardB := 0.U // Don't forward
  }
}
