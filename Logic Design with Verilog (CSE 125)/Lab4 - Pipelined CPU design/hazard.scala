...
class HazardUnit extends Module {
  ...
  when (io.idex_memread && (io.idex_rd === io.rs1 || io.idex_rd === io.rs2)) {
    io.pcwrite     := 2.U // last pc value
    io.ifid_bubble := true.B // insert a bubble
    io.idex_bubble := true.B // insert a bubble
  } 
  // branch flush
  .elsewhen (io.exmem_taken)
  {
    io.pcwrite := 1.U // next pc from mem stage
    io.ifid_flush  := true.B // insert a bubble
    io.idex_bubble  := true.B // insert a bubble
    io.exmem_bubble := true.B // insert a bubble
  }
}
